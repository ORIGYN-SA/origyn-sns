...
export type Period = 'all' | 'yearly' | 'monthly' | 'daily'
...
const useGetActivityStats = ({
  ...
  useEffect(() => {
    ...
    if (isSuccess && dynamicResponse) {
      const mappedDynamicData: MappedActivityData[] = dynamicResponse.map(r => {
        const baseNumber = Number(r.total_unique_accounts)
        const datetime = DateTime.fromMillis(Number(r.start_time) / 1_000_000)
