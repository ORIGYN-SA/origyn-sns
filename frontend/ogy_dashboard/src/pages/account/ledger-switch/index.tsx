...
const LedgerSwitch: FC<LedgerSwitchProps> = ({ className }) => {
  ...
  const cards = [
    {
      title: t("account.ledgerSwitch.steps.withdraw.title"),
      subtitle: (
        <div className="mt-2 text-sm leading-6 text-muted">
          {t("account.ledgerSwitch.steps.withdraw.subtitle")}
        </div>
      ),
    ...
    {
      title: t("account.ledgerSwitch.steps.deposit.title"),
      subtitle: (
        <div className="mt-2 text-sm leading-6 text-muted">
          {t("account.ledgerSwitch.steps.deposit.subtitle")}
        </div>
      ),
      children: <Deposit />,
      icon: <DepositIcon />,
    ...
    {
      title: t("account.ledgerSwitch.steps.swap.title"),
      subtitle: (
        <div className="mt-2 text-sm leading-6 text-muted">
          {t("account.ledgerSwitch.steps.swap.subtitle")}
        </div>
      ),
      children: <SwapTokens />,
      icon: <SwapIcon />,
      ...
      <div>
        <LedgerSwitchBanner className="!rounded-b-none !shadow-none">
          <div className="w-full flex justify-center mt-8">
            ...
            >
              {t("account.ledgerSwitch.howToSwap")}
            </ExternalLink>
          </div>
        </LedgerSwitchBanner>
        ...
        <div className="rounded-b-[40px] border-x border-b border-border-strong bg-surface-1 transform-gpu">
          <Disclosure>
            {({ open }) => (
              <>
                <Disclosure.Button className="group flex w-full items-center justify-between gap-4 border-t border-border-strong px-5 py-4 text-start sm:px-6">
                  <span className="text-[16px] font-medium leading-tight text-content sm:text-[17px]">
                    {t("account.ledgerSwitch.disclosureToggle")}
                  </span>
