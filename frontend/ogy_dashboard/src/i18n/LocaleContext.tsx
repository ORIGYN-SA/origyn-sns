...
// Returns a path builder bound to the active locale, so call sites can write
// `lp("/governance")` instead of threading the locale through every link.
export function useLocalePath(): (target: string) => string {
  const { locale } = useLocale();
  return useCallback((target: string) => localePath(locale, target), [locale]);
}
