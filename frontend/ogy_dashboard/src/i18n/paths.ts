...
// Build a localized in-app href. `target` may be relative ("account", "/token")
// or absolute ("/governance/neurons"); the function always returns
// "/<locale>/<path>".
...
export const splitLocalePath = (
  ...
  return isLocale(head)
    ? { locale: head, rest: tail }
    : { locale: null, rest: trimmed };
};
