...
pub use ogy_token_swap_api::updates::recover_stuck_transfer::{
    Args as RecoverStuckTransferArgs, Response as RecoverStuckTransferResponse,
};
...
pub(crate) async fn recover_stuck_transfer_impl(
    ...
    if let Some(info) = read_state(|s| s.data.token_swap.get_swap_info(block_index).cloned()) {
        match info.status {
            SwapStatus::TransferRequest(data) => _transfer_request_args = data,
            status => {
                return Err(RecoverStuckTransferResponse::SwapIsNotStuckInTransfer(
                    status,
                ))
            }
        }
    ...
    match recover_mode {
        RecoverTransferMode::RetryTransfer => transfer_new_token(block_index)
            .await
            .map_err(|err| RecoverStuckTransferResponse::FinalTransferFailed(err)),
