...
use crate::{
    client::{
        ...
        ogy_token_swap::{
            client::{
                deposit_account, get_whitelisted_principals_call, manipulate_swap_status,
                recover_stuck_burn_call, recover_stuck_transfer_call, requesting_principals,
                swap_info, swap_tokens_anonymous_call, swap_tokens_authenticated_call,
                swapping_statistics, update_whitelist_call,
            },
            get_swap_info,
        },
    },
...
fn valid_swap() {
    ...
    let whitelisted_principals =
        get_whitelisted_principals_call(&mut pic, controller, ogy_token_swap_canister_id);
...
fn test_retry_transfer_when_new_ledger_inactive() {
    ...
    .unwrap();
    pic.stop_canister(ogy_new_ledger_canister, Some(controller))
        .unwrap(); // stop the new ledger - this should make the third step ( transfer fail )
    ...
    let res = get_swap_info(
        ...
        Principal::anonymous(),
        ogy_token_swap_canister_id,
        &(ogy_token_swap_api::get_swap_info::Args {
    ...
    pic.start_canister(ogy_new_ledger_canister, Some(controller))
        .unwrap();
    pic.advance_time(Duration::from_secs(60));
    let result = swap_tokens_authenticated_call(
        &mut pic,
...
#[test]
#[should_panic(
    expected = "FATAL ERROR: PocketIC returned a rejection error: reject code CanisterReject, reject message Caller is not an authorised principal, error code CanisterRejectedMessage"
)]
