...
fn valid_transfer() {
    ...
    assert_eq!(
        mint_ogy(&mut pic, controller, ledger_canister_id, user1, amount),
        Ok(0)
    );

    assert_eq!(
        balance_of(&pic, ledger_canister_id, user1.to_string()),
    ...
    assert_eq!(
        balance_of(&pic, ledger_canister_id, user2.to_string()),
        Tokens::from_e8s(amount)
    );
}
