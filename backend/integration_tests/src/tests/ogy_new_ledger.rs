...
use crate::{
    client::icrc1::happy_path::{ balance_of, transfer },
    init::init,
...
fn valid_transfer() {
    ...
    assert_eq!(
        transfer(&mut pic, minting_account, ledger_canister_id, user1, amount.into()),
        Ok((0u8).into())
    );
    ...
    assert_eq!(
        transfer(&mut pic, user1.owner, ledger_canister_id, user2, amount.into()),
